...
describe 'ShowTodo opening panes and executing commands', ->
  ...
  executeCommand = (callback) ->
    ...
    atom.commands.dispatch(workspaceElement, 'todog:find-in-workspace')
  ...
  beforeEach ->
    ...
    activationPromise = atom.packages.activatePackage('todog').then (opts) ->
      showTodoModule = opts.mainModule

  describe 'when the todog:find-in-workspace event is triggered', ->
    it 'attaches and toggles the pane view in dock', ->
      dock = atom.workspace.getRightDock()
      expect(atom.packages.loadedPackages['todog']).toBeDefined()
  ...
  describe 'when export button is clicked', ->
    it 'export the list of todos in markdown and opens it', ->
      ...
      atom.config.set 'todog-atom.sortBy', 'Type'
    ...
    it 'export another list sorted differently in markdown', ->
      atom.config.set 'todog-atom.findTheseTodos', ['TODO']
      atom.config.set 'todog-atom.showInTable', ['Text', 'Type', 'File', 'Line']
      atom.config.set 'todog-atom.sortBy', 'File'
  ...
  describe 'when the todog:find-in-open-files event is triggered', ->
    beforeEach ->
      atom.commands.dispatch(workspaceElement, 'todog:find-in-open-files')
      waitsForPromise -> activationPromise
      runs ->
        waitsFor ->
          !showTodoModule.showTodoView.isSearching() and showTodoModule.showTodoView.isVisible()
  ...
  describe 'status bar indicator', ->
    ...
    it 'shows the current number of todos', ->
      ...
      executeCommand ->
        ...
        atom.config.set('todog-atom.statusBarIndicator', true)
