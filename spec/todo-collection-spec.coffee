...
describe 'Todo Collection', ->
  ...
  describe 'fetchRegexItem(todoRegex, activeProjectOnly)', ->
    ...
    it 'still respects ignored paths', ->
      atom.config.set('todog-atom.ignoreThesePaths', ['sample.js'])
      waitsForPromise ->
  ...
  describe 'ignore path rules', ->
    it 'works with no paths added', ->
      atom.config.set('todog-atom.ignoreThesePaths', [])
      waitsForPromise ->
        collection.fetchRegexItem(todoRegex)
      runs ->
        expect(collection.todos).toHaveLength 4
    ...
    it 'must be an array', ->
      ...
      atom.config.set('todog-atom.ignoreThesePaths', '123')
      waitsForPromise ->
        collection.fetchRegexItem(todoRegex)
      runs ->
        expect(collection.todos).toHaveLength 4
    ...
    it 'respects ignored directories and filetypes', ->
      ...
      atom.config.set('todog-atom.ignoreThesePaths', ['sample1', '*.md'])
    ...
    it 'respects ignored wildcard directories', ->
      atom.project.setPaths [fixturesPath]
      atom.config.set('todog-atom.ignoreThesePaths', ['**/sample.js', '**/sample.txt', '*.md'])
    ...
    it 'respects more advanced ignores', ->
      atom.project.setPaths [fixturesPath]
      atom.config.set('todog-atom.ignoreThesePaths', ['output(-grouped)?\\.*', '*1/**'])
  ...
  describe 'Sort todos', ->
    ...
    beforeEach ->
      ...
      atom.config.set 'todog-atom.findTheseTodos', ['FIXME', 'TODO']
    ...
    it 'sorts type in the defined order', ->
      ...
      atom.config.set 'todog-atom.findTheseTodos', ['TODO', 'FIXME']
      collection.sortTodos(sortBy: 'Type', sortAsc: true)
      expect(collection.todos[0].text).toBe 'todo 1'
      expect(collection.todos[1].text).toBe 'fixme 1'
      expect(collection.todos[2].text).toBe 'fixme 2'
  ...
  describe 'Filter todos', ->
    ...
    beforeEach ->
      atom.config.set 'todog-atom.showInTable', defaultShowInTable
      addTestTodos()
  ...
  describe 'Markdown', ->
    ...
    it 'creates markdown with different items', ->
      addTestTodos()
      atom.config.set 'todog-atom.showInTable', ['Type', 'File', 'Range']
      expect(collection.getMarkdown()).toEqual """
    ...
    it 'creates markdown as table', ->
      addTestTodos()
      atom.config.set 'todog-atom.exportAs', 'Table'
      expect(collection.getMarkdown()).toEqual """
        | Text | Type | File |
        |--------------------|
        | fixme 1 | __FIXME__ | [file1.txt](file1.txt) |
    ...
    it 'accepts missing ranges and paths in regexes', ->
      ...
      atom.config.set 'todog-atom.showInTable', ['Type', 'File', 'Range', 'Text']
      markdown = '\n## Unknown File\n\n- fixme 1 `FIXMEs`\n'
      expect(collection.getMarkdown()).toEqual """
        - __FIXME__ fixme 1\n
      """
    ...
    it 'accepts missing title in regexes', ->
      ...
      atom.config.set 'todog-atom.showInTable', ['Title']
      expect(collection.getMarkdown()).toEqual """
        - No details\n
      """
    ...
    it 'accepts missing items in table output', ->
      ...
      atom.config.set 'todog-atom.showInTable', ['Line']
      expect(collection.getMarkdown()).toEqual """
        | Line |
        |------|
        | |\n
