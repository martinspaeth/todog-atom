...
describe "Show Todo View", ->
  ...
  beforeEach ->
    ...
    atom.config.set 'todog-atom.findUsingRegex', '/\\b(${TODOS}):?\\d*($|\\s.*$)/g'
    atom.config.set 'todog-atom.autoRefresh', true
  ...
  describe "Automatic update of todos", ->
    ...
    it "can stop auto refreshing", ->
      atom.config.set 'todog-atom.autoRefresh', false
