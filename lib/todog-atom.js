...
class TodogAtom {
  ...
  activate() {
    ...
    this.disposables.add(atom.commands.add('atom-workspace', {
      'todog:toggle': (event) => this.show(undefined, event),
      'todog:find-in-workspace': () => this.show('workspace'),
      'todog:find-in-project': () => this.show('project'),
      'todog:find-in-open-files': () => this.show('open'),
      'todog:find-in-active-file': () => this.show('active')
    }))
  ...
  createCollection() {
    ...
    const config = atom.config.getSchema('todog-atom')
    if (config) {
  ...
  consumeStatusBar(statusBar) {
    atom.config.observe('todog-atom.statusBarIndicator', newValue => {
      if (newValue) {
